...
class PrepareVoiceoverPromptWorker(WorkerThread):
    """Worker để tạo input prompt cho voiceover từ template + topic + language + scene range"""
    
    def __init__(self, topic, language, scene_from, scene_to, template_path, output_path):
        super().__init__()
        self.topic = topic
    ...
    def run(self):
        try:
            ...
            self.log(f"🎬 Scene Range: {self.scene_from}-{self.scene_to}")
            ...
            self.log("⚙️ Filling topic, language, scene range into template...")
            prompt = template.replace(
                "[Điền chủ đề chi tiết. VD: Câu chuyện thành lập Virgin Atlantic và myth 0 đồng]",
                self.topic
            )
            ...
            prompt = prompt.replace(
                "[VD: 1-5]",
                f"[{self.scene_from}-{self.scene_to}]"
            )
        ...
        while not self._stop_requested:
            ...
            try:
                ...
                if not self._send_prompt_and_verify(input_box, prompt_text):
                    self.log("❌ Không thể gửi prompt, restart browser...")
                ...
                if success:
                    ...
                    if self.auto_restart_on_timeout:
                        self.log("🔄 Auto restart: Sẽ restart browser và thử lại...")
                        # Cleanup before restart
                        if self.driver:
                            try:
                                self.driver.quit()
