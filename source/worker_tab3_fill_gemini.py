        ...
        try:
            ...
            while i < len(prompts):
                ...
                if need_restart:
                    ...
                    self.driver.get(self.gemini_url)
                    time.sleep(5)
                    self.log("✅ Browser đã restart xong")
                    need_restart = False
                ...
                try:
                    ...
                    if not self._send_prompt_and_verify(input_box, prompt_text):
                        ...
                        time.sleep(2)
                        need_restart = True
                        continue  # Restart và retry scene này
                    ...
                    if not success:
                        ...
                        if self.auto_restart_on_timeout:
                            # Re-check progress từ file output
                            last_scene, self.processed_files = self._get_last_completed_scene()
                            self.log(f"🔄 Auto restart: Restart browser và tiếp tục từ SCENE {last_scene + 1}...")
                            self.log(f"   Đã hoàn thành: {len(self.processed_files)} files")
                            # Close current browser
                            try:
                                self.driver.quit()
                            except:
